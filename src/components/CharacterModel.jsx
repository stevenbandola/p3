...
export default function CharacterModel({ characterUrl = '', sharePos = false, player }) {
  ...
  // const model = useLoader(GLTFLoader, characterUrl)
  const defaultModel = useGLTF('https://models.readyplayer.me/64f0265b1db75f90dcfd9e2c.glb')
  const defaultClone = useMemo(() => SkeletonUtils.clone(defaultModel.scene), [defaultModel.scene])
  ...
  return (
    <group position={[0, -0.9, 0]} ref={group} name={`character-${player.id}`} dispose={null}>
      ...
      {customClone ? <primitive object={customClone} /> : <primitive object={defaultClone} />}
    </group>
  )
}
